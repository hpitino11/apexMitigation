<!-- theme preloader start -->
@include('frontend.includes.partials.preloader')
<!-- theme preloader end -->

    <!-- JS here -->
    <script defer src="{{ asset('/assets/js/jquery-3.6.0.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/ajax-form.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.appear.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.circleType.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.lettering.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.magnific-popup.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.nice-select.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery.odometer.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/jquery-sidebar-content.js') }}"></script>
    <script defer src="{{ asset('/assets/js/nouislider.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/swiper.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/wow.min.js') }}"></script>
    <script defer src="{{ asset('/assets/js/main.js') }}"></script>